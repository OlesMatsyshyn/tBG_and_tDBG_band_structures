...
v = VideoWriter('tDBG.avi');
v.FrameRate = 10;
open(v) ;

for al = 0.4:0.005:0.7
    theta  = al * pi / 180; % twist angle in radians
    ...
    textprogressbar(join(...
          ['>> calculating the band structure for theta = '...
                                     num2str(al,'%4.3f') ', progress: ']));
    ...
    title(join(["\theta = " num2str(al,'%4.3f'), '; U_z[d] = 0']))
    ...
    F = getframe(hfig); % Capture the frame
    %F = getframe ;     % this one ignores axes
    writeVideo(v,F)     % add the frame to the movie
    close
end
close(v)
